...
def mutation(shape, prob=None):
    ...
    choiceSet = choice(numOptions, numOfMut, replace=False,
                       p=prob)  # <-Change probabilities here
...
def _legal_RGB_value(RGBValue, RGBRange):
    ...
    if smallestDiff >= RGBRange / 2:
        ...
        RGBValue += random.randint((-RGBRange / 2) -
                                   ((RGBRange / 2) - rDiffFromMax), rDiffFromMax)
    elif rDiffFromMin < RGBRange / 2:
        RGBValue += random.randint(-rDiffFromMin,
