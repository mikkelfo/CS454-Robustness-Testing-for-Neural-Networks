...
def mutate_rgb_values():
    pass


def mutation(shape_mut):
    change_shape = False
    change_cenpoint = False
    change_rgb = False

    # Decide how many things will be mutated
    num_of_mut = random.randint(1, 3)

    num_set = {1, 2, 3}

    # Pull out the things that will be mutated and set their value to 'True'
    choice_set = random.sample(num_set, num_of_mut)

    if 1 in choice_set:
        change_shape = True
    if 2 in choice_set:
        change_cenpoint = True
    if 3 in choice_set:
        change_rgb = True
    ...
    if change_cenpoint:
        shape_mut.center = createRandomPoint(shape_mut.center)
